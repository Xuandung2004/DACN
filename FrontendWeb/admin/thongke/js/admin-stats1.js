...
// --- 1. Doanh thu theo tháng (Bar Chart) ---
    ...
    document.getElementById('btnFetchTongQuan')?.addEventListener('click', async () => {
        ...
        if (data && data.tongDoanhThu !== undefined) {
            ...
            resultElement.innerHTML = `
                ...
                <span class="text-primary">Số đơn hàng: ${data.soLuongDonHangHoanThanh.toLocaleString()}</span>
            `;
        } else {
    ...
    document.getElementById('btnFetchWeeklyDetail')?.addEventListener('click', async () => {
        ...
        if (data && data.tongDoanhThu !== undefined) {
            ...
            resultElement.innerHTML = `
                ...
                <span class="text-dark">${formattedRevenue}</span> <br>
                <span class="text-info">Số đơn hàng: ${data.soDonHang.toLocaleString()}</span>
            `;
        } else {
            resultElement.innerHTML = `<span class="text-danger">Không tìm thấy dữ liệu chi tiết tuần ${tuanNam} hoặc lỗi API.</span>`;
        }
    });

    // 3. Chi Tiết Tháng (MỚI - Endpoint: /ChiTietThang)
    document.getElementById('btnFetchMonthlyDetail')?.addEventListener('click', async () => {
        ...
        if (!month || !year || month < 1 || month > 12 || year < 2000) {
            resultElement.innerHTML = '<span class="text-danger">Vui lòng nhập Tháng và Năm hợp lệ.</span>';
            return;
        }
        ...
        if (data && data.tongDoanhThu !== undefined) {
            const formattedRevenue = formatCurrency(data.tongDoanhThu);
            resultElement.innerHTML = `
                <span class="text-warning">Doanh thu Tháng ${thangNam} (${data.thoiGian}): </span><br>
                <span class="text-dark">${formattedRevenue}</span> <br>
                <span class="text-warning">Số đơn hàng: ${data.soDonHang.toLocaleString()}</span>
            `;
        } else {
    ...
    document.getElementById('btnFetchYearlyDetail')?.addEventListener('click', async () => {
        ...
        if (data && data.tongDoanhThu !== undefined) {
            ...
            resultElement.innerHTML = `
                ...
                <span class="text-dark">${formattedRevenue}</span> <br>
                <span class="text-success">Tổng đơn hàng: ${data.soLuongDonHangHoanThanh.toLocaleString()}</span>
            `;
        } else {
